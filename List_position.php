...
<html lang="en">
    <head>
        ...
        <title>Position List | Trans-Master APARORS</title>
    ...
    <body class="sb-nav-fixed">
                ...
                <main>
                    <div class="container-fluid px-4 mt-3">
                            <h5><ol class="breadcrumb mb-4">
                                <li class="breadcrumb-item"><a  id="a" href="List_employee.php">Employee</a></li>
                                <li class="breadcrumb-item"><a  href="">Position</a></li>
                            </ol> </h5> 
                        <button type="button" class="btn btn-success btn-sm mb-3" data-bs-toggle="modal" data-bs-target="#add_position_modal">
                        <i class="fa-solid fa-circle-plus"></i> Position
                        </button>
                        <div class="card mb-4">
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                         <tr>
                                            ...
                                            <td>
                                                <button type="submit" class="btn btn-primary  btn-sm updateposition" value="<?= $row['position_id'];?>"><i class='fas fa-edit'></i></button>
                                                <button type="submit" class="btn btn-danger btn-sm deleteposition" value="<?= $row['position_id'];?>"><i class='fas fa-trash'></i></button>
                                            </td>
                                        </tr>
                                        <?php }?>   
                                    </tbody>
        ...
        <!-- add -->
        <div class="modal fade" id="add_position_modal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                ...
                <div class="modal-content"> 
                    ...
                    <form action='position-crud.php' autocomplete="off" enctype="multipart/form-data" id="pos_form" method="POST">
                        <div class="modal-body">
                            <div class="row mb-4">
                                <div class="col-sm-12">
                                    <label class="form-label" for="firstName">Position name</label>
                                    <input type="text" name="post_name" id="posname" placeholder="Enter position name" class=" form-control" />
                                </div>
                            </div> 
                        </div>
        ...
        <!-- update -->
        <div class="modal fade" id="position_update_modal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content"> 
                    <div class="modal-header">
                    ...
                    <form action='position-crud.php' autocomplete="off" enctype="multipart/form-data" id="updateform" method="POST">
                        <div class="modal-body">
                            <div class="row mb-4">
                                <div class="col-sm-12">
                                    <input type="hidden"  name="postid" id="post_id" >
                                    <label class="form-label" for="firstName">Position name</label>
                                    <input type="text" name="post_name" id="post_name" placeholder="Enter position name"class=" form-control" />
                                </div>
                            </div> 
                        </div>
                        <div class="modal-footer">
...
<script>
    ...
    $(document).on('submit', '#pos_form', function (e) {
        ...
        formData.append("add_position", true);
    
        $.ajax({
            type: "POST",
            url: "position-crud.php",
            data: formData,
            processData: false,
            contentType: false,
    ...
    $(document).on('click', '.updateposition', function () {
        ...
        $.ajax({
            type: "GET",
            url: "position-crud.php?post_id=" + post_id,
            success: function (response) {
                ...
                if(res.status == 404)
                {
                    ...
                    $('#post_id').val(res.data.position_id);
                    $('#post_name').val(res.data.position_name);
    
                    $('#position_update_modal').modal('show');
                }
            }
        });
    }); 
...
$(document).on('submit', '#updateform', function (e) {
    ...
    var formData = new FormData(this);
    formData.append("update_position", true);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            window.location.reload();
                ...
                $('#position_update_modal').modal('hide');
                $('#updateform')[0].reset();
